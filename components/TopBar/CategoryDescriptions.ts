...
const categoryDescriptions: CategoryDescriptions = {
  //frontend
  accessibility: 'Accessibility is the concept of whether a product or service can be used by everyone—however they encounter it. Even though accessibility laws exist for people with disabilities, designers should try to accommodate all potential users in many contexts of use anyway. ',
  animations: 'Animation is a method of photographing successive drawings, models, or even puppets, to create an illusion of movement in a sequence.',
  colors: 'Colors are a factor that impacts the appearance of objects and light sources. These sources tend to be described in terms of hue, lightness, and saturation.',
  'design inspiration': 'Sometimes you need some help getting inspired. Luckily, there are a TON of great design resources to help spark some ideas.',
  ...
  react: 'React is a JavaScript library for building user interfaces. It was developed by Facebook and is used by many popular websites such as Netflix, Airbnb, and Instagram. React also allows developers to create reusable UI components and makes it easy to manage the state of an application.',
  ...
  hosting: 'Web hosting is an online service that makes your content's website accessible on the internet. When purchasing a hosting plan, you are renting space on a physical server to store all the files and data.',
  officialdocs: 'Official docs is short for official documentation. This is an important resource for users of software products and services as it provides a comprehensive guide to using the software.',
  ...
  'game development': 'Game development is the process of creating games for various platforms. It involves different stages and aspects, such as design, development, release, storyboarding, writing, audio, art, coding, music, animation, modeling, testing, and marketing. ',
  ...
  youtubeTesting: 'Testing is executing a system in order to identify any gaps, errors, or missing requirements For example, YouTube uses A/B testing to test different versions of their website to see which version performs better.',
