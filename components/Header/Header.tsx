    ...
    navLinks.map(({ inActiveIcon, activeIcon, label, href }, i) => (
      <li key={i}>
        <a
          href={href}
          className={clsx(
            'hover:bg-slate-100 hover:bg-opacity-50 dark:hover:bg-zinc-400 dark:hover:bg-opacity-10 flex items-center justify-start p-2 gap-2 text-base font-medium leading-5 rounded-xl',
            router.asPath === href
        ...
        >
          <span className="flex items-center justify-center" title={label}>
            {router.asPath === href ? activeIcon : inActiveIcon}
          </span>
          <span>{label}</span>
        </a>
      </li>
